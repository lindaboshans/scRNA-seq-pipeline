...
# set directory
Seurat.list <- c('DS-Ngn2', 'DS-AD3', 'DS-AD7', "Wk4")

# Initialize an empty list to store Seurat objects
Seurat.objects <- list()

for (i in 1:length(Seurat.list)) {
  directory = paste0("~/Documents/Ruiqi_scRNAseq/10x_July2023/", Seurat.list[i], "/filtered_feature_bc_matrix/")
  # load expression matrix for editing
  expression_matrix <- ReadMtx(
    mtx = paste0(directory, "matrix.mtx.gz"), 
  ...
  # Remove 'GRCh38_' prefix from the gene names
  rownames(exp_mtx) <- gsub("GRCh38_", "", rownames(exp_mtx))
  # Verify changes
  head(rownames(exp_mtx))
  # Initialize the Seurat object with the raw (non-normalized data).
  Seurat.object <- CreateSeuratObject(counts = exp_mtx, project = Seurat.list[i], min.cells = 3, min.features = 200)
  Seurat.object[["percent.mt"]] <- PercentageFeatureSet(Seurat.object, pattern = "^MT-")
  # Visualize QC metrics as a violin plot
  png(paste0("~/Documents/Ruiqi_scRNAseq/10x_July2023/", Seurat.list[i], "_violinplot_pre_mito_filt_20240223.png"), width = 8, height = 5, units = 'in', res = 300)
  VlnPlot(Seurat.object, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3)
  dev.off()
  Seurat.object <- subset(Seurat.object, subset = nFeature_RNA > 500 & nFeature_RNA < 6000 & percent.mt < 5)
  ...
  Seurat.object <- RunPCA(Seurat.object, features = VariableFeatures(object = Seurat.object), dims = 1:20)
  png(paste0("~/Documents/Ruiqi_scRNAseq/10x_July2023/", Seurat.list[i], "_elbowplot_20240223.png"), width = 5, height = 5, units = 'in', res = 300)
  ...
  png(paste0("~/Documents/Ruiqi_scRNAseq/10x_July2023/", Seurat.list[i], "_DimPlot_umap_by_clusters_20240223.png"), width = 6, height = 4, units = 'in', res = 300)
  DimPlot(Seurat.object, reduction = "umap", group.by = "seurat_clusters")
  dev.off()
  # Assign the processed Seurat object to the list with a dynamic name
  Seurat.objects[[Seurat.list[i]]] <- Seurat.object
  # Generate filename
  Filename.brief <- paste0("~/Desktop/", Seurat.list[i], '_mito10_scaled_20240225.rds')
  saveRDS(Seurat.object, file = Filename.brief)
}

# Access a Seurat object by name
Ngn2 <- Seurat.objects[["DS-Ngn2"]]
AD3 <- Seurat.objects[["DS-AD3"]]
AD7 <- Seurat.objects[["DS-AD7"]]
Wk4 <- Seurat.objects[["Wk4"]]
